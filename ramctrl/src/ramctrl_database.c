...
static const char *sql_promote_node = 
    "SELECT ram.promote_node($1);";

static const char *sql_demote_node = 
    "SELECT ram.demote_node($1);";

static const char *sql_add_node = 
    "SELECT ram.add_node($1, $2, $3);";

static const char *sql_remove_node = 
    "SELECT ram.remove_node($1);";

static const char *sql_set_maintenance = 
    "SELECT ram.set_maintenance_mode($1, $2);";

static const char *sql_trigger_failover = 
    "SELECT ram.trigger_failover($1);";

static PGconn *
ramctrl_connect_database(ramctrl_context_t *ctx)
{
    char conninfo[1024];
    PGconn *conn;
    ...
    PGresult *res = PQexec(conn, "SELECT 1 FROM pg_extension WHERE extname = 'pg_ram';");
...
bool
ramctrl_get_cluster_info(ramctrl_context_t *ctx, ramctrl_cluster_info_t *cluster_info)
{
    PGconn *conn;
    PGresult *res;
    bool result = false;
...
bool
ramctrl_get_nodes_info(ramctrl_context_t *ctx, ramctrl_node_info_t *nodes, int32_t *node_count, int32_t max_nodes)
{
    PGconn *conn;
    PGresult *res;
    bool result = false;
...
bool
ramctrl_add_node(ramctrl_context_t *ctx, int32_t node_id, const char *hostname, int32_t port)
{
    PGconn *conn;
    PGresult *res;
    bool result = false;
...
bool
ramctrl_remove_node(ramctrl_context_t *ctx, int32_t node_id)
{
    PGconn *conn;
    PGresult *res;
    bool result = false;
...
bool
ramctrl_set_maintenance_mode(ramctrl_context_t *ctx, int32_t node_id, bool enable)
{
    PGconn *conn;
    PGresult *res;
    bool result = false;
    ...
    res = PQexecParams(conn, sql_set_maintenance, 2, NULL, params, NULL, NULL, 0);
    if (PQresultStatus(res) == PGRES_TUPLES_OK)
    {
        if (ctx->verbose)
            printf("ramctrl: Node %d maintenance mode %s\n", 
                   node_id, enable ? "enabled" : "disabled");
        result = true;
    }
    else if (ctx->verbose)
    {
...
ramctrl_trigger_failover(ramctrl_context_t *ctx, int32_t target_node_id)
{
    ...
    if (target_node_id > 0)
    {
        ...
        params[0] = node_id_str;
        res = PQexecParams(conn, sql_trigger_failover, 1, NULL, params, NULL, NULL, 0);
    }
    else
    {
...
ramctrl_get_node_info(ramctrl_context_t *ctx, ramctrl_node_info_t *nodes, int32_t *node_count)
{
    ...
    *node_count = 0;
    ...
    if (PQresultStatus(res) == PGRES_TUPLES_OK)
    {
        ...
        if (ntuples > 0)
        {
            int count = (ntuples < RAMCTRL_MAX_NODES) ? ntuples : RAMCTRL_MAX_NODES;
            *node_count = count;
...
bool 
ramctrl_enable_maintenance_mode(ramctrl_context_t *ctx, int32_t node_id)
{
    ...
    if (!ramctrl_database_connect(&conn, "localhost", 5432, "postgres", "postgres", NULL))
        return false;
...
bool 
ramctrl_disable_maintenance_mode(ramctrl_context_t *ctx, int32_t node_id)
{
    ramctrl_database_connection_t conn;
    bool result;
...
ramctrl_get_all_nodes(ramctrl_context_t *ctx, ramctrl_node_info_t **nodes, int *node_count)
{
    ...
    if (result && count > 0)
    {
        *nodes = malloc((size_t)count * sizeof(ramctrl_node_info_t));
        if (*nodes)
        {
            memcpy(*nodes, static_nodes, (size_t)count * sizeof(ramctrl_node_info_t));
            *node_count = count;
        }
        else
        {
...
bool 
ramctrl_database_connect(ramctrl_database_connection_t *conn,
...
void 
ramctrl_database_disconnect(ramctrl_database_connection_t *conn)
{
    if (!conn)
        return;
...
bool 
ramctrl_database_reconnect(ramctrl_database_connection_t *conn)
{
    if (!conn)
        return false;
...
bool 
ramctrl_database_get_all_nodes(ramctrl_database_connection_t *conn,
    ...
    if (PQresultStatus(res) == PGRES_TUPLES_OK)
    {
        ...
        *node_count = (num_rows > max_nodes) ? max_nodes : num_rows;
        ...
        for (i = 0; i < *node_count; i++)
        {
            ...
            node->node_id = atoi(PQgetvalue(res, i, 0));
            strncpy(node->hostname, PQgetvalue(res, i, 1), sizeof(node->hostname) - 1);
            node->port = atoi(PQgetvalue(res, i, 2));
...
bool 
ramctrl_database_promote_node(ramctrl_database_connection_t *conn, int32_t node_id)
{
    char query[256];
    PGresult *res;
    bool result = false;
...
bool 
ramctrl_database_demote_node(ramctrl_database_connection_t *conn, int32_t node_id)
{
    char query[256];
    PGresult *res;
    bool result = false;
...
bool 
ramctrl_database_trigger_failover(ramctrl_database_connection_t *conn, int32_t target_node_id)
{
    char query[256];
    PGresult *res;
    bool result = false;
    ...
    snprintf(query, sizeof(query), "SELECT ram.trigger_failover(%d)", target_node_id);
    res = PQexec((PGconn*)conn->connection, query);
...
bool 
ramctrl_database_add_node(ramctrl_database_connection_t *conn,
                         int32_t node_id, const char *hostname, int32_t port)
{
    char query[512];
    PGresult *res;
    bool result = false;
    ...
    snprintf(query, sizeof(query), "SELECT ram.add_node(%d, '%s', %d)", 
             node_id, hostname, port);
    res = PQexec((PGconn*)conn->connection, query);
...
bool 
ramctrl_database_remove_node(ramctrl_database_connection_t *conn, int32_t node_id)
{
    char query[256];
    PGresult *res;
    bool result = false;
...
bool 
ramctrl_database_enable_maintenance(ramctrl_database_connection_t *conn, int32_t node_id)
{
    char query[256];
    PGresult *res;
    bool result = false;
    ...
    snprintf(query, sizeof(query), "SELECT ram.set_maintenance_mode(%d, true)", node_id);
    res = PQexec((PGconn*)conn->connection, query);
...
bool 
ramctrl_database_disable_maintenance(ramctrl_database_connection_t *conn, int32_t node_id)
{
    char query[256];
    PGresult *res;
    bool result = false;
    ...
    snprintf(query, sizeof(query), "SELECT ram.set_maintenance_mode(%d, false)", node_id);
    res = PQexec((PGconn*)conn->connection, query);
