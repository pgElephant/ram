/*
 * ram/ramctrl/src/ramctrl_common.c
...
void
cli_pprint_json(const char *json_str)
{
    if (json_str == NULL)
    {
        printf("null\n");
...
bool
cli_common_getenv(const char *var_name, char *buffer, size_t buffer_size)
{
    const char *value;
...
bool
cli_common_getenv_pgsetup(char *pgdata, char *pgport, char *pguser, size_t buffer_size)
{
    bool success = true;
...
void
cli_common_get_set_pgdata_or_exit(char *pgdata, size_t buffer_size)
{
    struct stat st;
...
int
cli_common_keeper_getopts(int argc, char **argv, struct option *long_options,
...
int
cli_create_node_getopts(int argc, char **argv, struct option *long_options,
