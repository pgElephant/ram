...
void
ramctrl_table_print_header(const char *title)
{
	int			i;
...
void
ramctrl_table_print_separator(void)
{
	/* No separators - clean modern look */
}

void
ramctrl_table_print_row(const char *key, const char *value)
{
	if (!key || !value)
		return;
...
void
ramctrl_table_print_row_int(const char *key, int value)
{
	char		value_str[32];
...
void
ramctrl_table_print_row_bool(const char *key, bool value)
{
	ramctrl_table_print_row(key, value ? "Yes" : "No");
}

void
ramctrl_table_print_row_time(const char *key, time_t timestamp)
{
	char		time_str[64];
	struct tm	*tm_info;
...
void
ramctrl_table_print_footer(void)
{
	fprintf(stdout, "\n");
}

void
ramctrl_table_print_daemon_status(const ramctrl_daemon_status_t *status)
{
	if (!status)
		return;
	...
	if (status->is_running)
	{
		...
		ramctrl_table_print_row("Binary", "/usr/local/bin/ramd");
		ramctrl_table_print_row("Config", "/etc/ramd/ramd.conf");
		ramctrl_table_print_row("Log File", "/var/log/ramd/ramd.log");
		ramctrl_table_print_row("PID File", "/var/run/ramd/ramd.pid");
	}
	else
	{
		ramctrl_table_print_row("Binary", "/usr/local/bin/ramd");
...
void
ramctrl_table_print_cluster_status(const ramctrl_cluster_info_t *cluster)
{
...
void
ramctrl_table_print_node_status(const ramctrl_node_info_t *node)
{
	char		lsn_str[32];
	char		lag_str[32];
