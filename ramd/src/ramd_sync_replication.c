...
ramd_sync_replication_set_mode(ramd_sync_mode_t mode)
{
    ...
    ramd_log_info("Synchronous replication mode set to: %s", ramd_sync_mode_to_string(mode));
...
ramd_sync_replication_add_standby(int32_t node_id, const char *application_name)
{
    ...
    strncpy(standby->application_name, application_name, sizeof(standby->application_name) - 1);
    ...
    ramd_log_info("Added synchronous standby: node %d (%s)", node_id, application_name);
    return ramd_sync_replication_update_postgresql_config();
}
...
ramd_sync_replication_remove_standby(int32_t node_id)
{
    ...
    if (!found)
    {
        ramd_log_warning("Synchronous standby node %d not found for removal", node_id);
        return false;
    }
...
ramd_sync_replication_get_status(ramd_sync_status_t *status)
{
    ...
    for (int i = 0; i < RAMD_MAX_NODES; i++)
    {
        if (g_sync_status.standbys[i].node_id > 0 && g_sync_status.standbys[i].is_sync)
        {
    ...
    if (g_sync_config.enforce_sync_standbys)
    {
        if (status.num_sync_standbys_connected < g_sync_config.num_sync_standbys)
        {
            ramd_log_warning("Insufficient synchronous standbys: %d connected, %d required",
                            status.num_sync_standbys_connected, g_sync_config.num_sync_standbys);
            return false;
        }
    }
...
ramd_sync_replication_update_postgresql_config(void)
{
    ...
    if (!ramd_sync_generate_standby_names(standby_names, sizeof(standby_names),
                                         g_sync_status.standbys, RAMD_MAX_NODES))
    {
    ...
    snprintf(config_sql, sizeof(config_sql),
             "ALTER SYSTEM SET synchronous_standby_names = '%s'", standby_names);
...
ramd_sync_generate_standby_names(char *output, size_t output_size, 
                                const ramd_sync_standby_t *standbys, int32_t count)
{
    ...
    {
        /* Multiple standbys */
        snprintf(temp, sizeof(temp), "FIRST %d (", g_sync_config.num_sync_standbys);
    }
    ...
    for (int32_t i = 0; i < count && sync_count < g_sync_config.num_sync_standbys; i++)
    {
        if (standbys[i].node_id > 0 && standbys[i].is_sync)
        {
            ...
            strncat(output, standbys[i].application_name, output_size - strlen(output) - 1);
            first = false;
            sync_count++;
        }
    }
                                     ...
                                     int32_t primary_port)
{
    ...
    snprintf(backup_cmd, sizeof(backup_cmd),
             ...
             primary_host, primary_port, config->postgresql_data_dir);
                                        ...
                                        int32_t primary_port)
{
    ...
    ramd_log_info("Configuring recovery for streaming replication from %s:%d", primary_host, primary_port);
    
    /* Create recovery.conf or update postgresql.auto.conf */
    char recovery_conf_path[512];
    snprintf(recovery_conf_path, sizeof(recovery_conf_path), "%s/recovery.conf", config->postgresql_data_dir);
    
    /* Check if we should use recovery.conf (PostgreSQL < 12) or postgresql.auto.conf (PostgreSQL >= 12) */
    bool use_recovery_conf = true; /* Default for older versions */
    
    /* Try to detect PostgreSQL version */
    ramd_postgresql_connection_t conn;
    if (ramd_postgresql_connect(&conn, "localhost", config->postgresql_port, "postgres", "postgres", ""))
    {
        PGresult *res = PQexec((PGconn*)conn.connection, "SHOW server_version_num");
        if (res && PQntuples(res) > 0)
        {
            int version_num = atoi(PQgetvalue(res, 0, 0));
            use_recovery_conf = (version_num < 120000); /* PostgreSQL 12+ uses postgresql.auto.conf */
            PQclear(res);
        }
    ...
    else
    {
        ...
        snprintf(auto_conf_path, sizeof(auto_conf_path), "%s/postgresql.auto.conf", config->postgresql_data_dir);
    ...
    snprintf(trigger_dir, sizeof(trigger_dir), "%s", config->postgresql_data_dir);
    
    /* Ensure the trigger file doesn't exist initially */
    char trigger_file[512];
    snprintf(trigger_file, sizeof(trigger_file), "%s/failover.trigger", trigger_dir);
    unlink(trigger_file);
                                     ...
                                     const char *application_name)
{
    ...
    ramd_log_info("Setting up streaming replication to primary %s:%d", primary_host, primary_port);
...
ramd_sync_replication_promote_standby(const ramd_config_t *config)
{
    ...
    char trigger_file[512];
    snprintf(trigger_file, sizeof(trigger_file), "%s/failover.trigger", config->postgresql_data_dir);
    ...
    if (ramd_postgresql_connect(&conn, "localhost", config->postgresql_port, "postgres", "postgres", ""))
    {
        ...
        if (res && PQntuples(res) > 0)
        {
            ...
            if (!is_recovering)
            {
                ...
                res = PQexec((PGconn*)conn.connection, "ALTER SYSTEM SET synchronous_standby_names = ''");
                if (res)
                {
                    PQclear(res);
