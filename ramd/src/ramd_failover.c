...
void
ramd_failover_context_set_reason(ramd_failover_context_t* context,
                                      const char* reason)
{
	if (!context || !reason)
		return;
...
bool
ramd_failover_should_trigger(const ramd_cluster_t* cluster,
...
bool
ramd_failover_select_new_primary(const ramd_cluster_t* cluster,
...
bool
ramd_failover_promote_node(ramd_cluster_t* cluster,
                                const ramd_config_t* config, int32_t node_id)
{
	...
	promotion_success = ramd_postgresql_promote(config);
...
bool
ramd_failover_stop_replication_on_node(const ramd_config_t* config,
                                            int32_t node_id)
{
	ramd_postgresql_connection_t conn;
	PGresult* res;
	...
	{
		res = ramd_query_exec_with_result((PGconn*) conn.connection, "SELECT pg_wal_replay_pause()");
		if (res)
		{
			PQclear(res);
		...
		res = ramd_query_exec_with_result((PGconn*) conn.connection, "SELECT pg_promote_node()");
		if (res)
		{
			PQclear(res);
...
bool
ramd_failover_update_sync_replication_config(ramd_cluster_t* cluster,
                                                  int32_t new_primary_id)
{
	...
	new_primary = ramd_cluster_find_node(cluster, new_primary_id);
	if (!new_primary)
		return false;
	...
	if (standby_count > 0)
	{
		...
		if (ramd_postgresql_connect(&conn, new_primary->hostname,
		                            ...
		                            "postgres", ""))
		{
			res = ramd_query_exec_with_result((PGconn*) conn.connection, config_update);
			if (res)
			{
				PQclear(res);
			...
			res = ramd_query_exec_with_result((PGconn*) conn.connection, "SELECT pg_reload_conf()");
			if (res)
			{
				PQclear(res);
				ramd_log_info(
...
bool
ramd_failover_rebuild_failed_replicas(ramd_cluster_t* cluster,
                                           const ramd_config_t* config)
{
	int rebuilt_count = 0;
	int i;
...
bool
ramd_failover_rebuild_replica_node(ramd_cluster_t* cluster,
                                        const ramd_config_t* config,
                                        int32_t node_id)
{
	ramd_node_t* node;
	                            ...
	                            "postgres", "postgres", ""))
	{
		...
		{
			is_recovering = (strcmp(PQgetvalue(res, 0, 0), "t") == 0);
			PQclear(res);
...
bool
ramd_failover_demote_failed_primary(ramd_cluster_t* cluster,
                                         const ramd_config_t* config,
                                         int32_t failed_node_id)
{
	ramd_node_t* failed_node;

	if (!cluster || !config)
		return false;

	failed_node = ramd_cluster_find_node(cluster, failed_node_id);
	if (!failed_node)
		return true;
...
bool
ramd_failover_update_standby_nodes(ramd_cluster_t* cluster,
                                        const ramd_config_t* config,
                                        int32_t new_primary_id)
{
	ramd_node_t* new_primary;
...
bool
ramd_failover_validate_promotion(const ramd_cluster_t* cluster,
                                      int32_t promoted_node_id)
{
	...
	promoted_node = ramd_cluster_find_node((ramd_cluster_t*) cluster, promoted_node_id);
	if (!promoted_node)
		return false;
...
bool
ramd_failover_recover_failed_node(ramd_cluster_t* cluster,
                                       const ramd_config_t* config,
                                       int32_t failed_node_id)
{
	ramd_node_t* failed_node;
...
bool
ramd_failover_take_basebackup(const ramd_config_t* config,
                                   ...
                                   int32_t primary_port)
{
	...
	conn = ramd_conn_get(primary_host, primary_port, config->database_name, 
...
bool
ramd_failover_configure_recovery(const ramd_config_t* config,
                                      const char* primary_host,
                                      int32_t primary_port)
{
