...
ramd_query_result_t
ramd_query_exec(PGconn *conn, const char *sql)
{
	PGresult *result;
...
PGresult *
ramd_query_exec_with_result(PGconn *conn, const char *sql)
{
	PGresult *result;
...
PGresult *
ramd_query_exec_params(PGconn *conn, const char *sql,
...
bool
ramd_query_result_ok(PGresult *result)
{
	if (!result)
		return false;
...
const char *
ramd_query_get_string_value(PGresult *result, int row, int col)
{
...
int
ramd_query_get_int_value(PGresult *result, int row, int col)
{
	const char *value;

	value = ramd_query_get_string_value(result, row, col);
	if (!value)
		return 0;
...
bool
ramd_query_get_bool_value(PGresult *result, int row, int col)
{
	const char *value;
...
char *
ramd_query_get_single_string(PGconn *conn, const char *sql)
{
	PGresult   *result;
	char       *value = NULL;
	...
	{
		str_value = PQgetvalue(result, 0, 0);
		if (str_value)
		{
...
int
ramd_query_get_single_int(PGconn *conn, const char *sql)
{
	PGresult *result;
	int       value = -1;
...
bool
ramd_query_get_single_bool(PGconn *conn, const char *sql)
{
	PGresult *result;
	bool      value = false;
...
bool
ramd_query_is_in_recovery(PGconn *conn)
{
	if (!conn)
		return false;
...
bool
ramd_query_is_accepting_connections(PGconn *conn)
{
	PGresult *result;
	bool      success;
	...
	result = ramd_query_exec_with_result(conn, "SELECT 1");
	if (result)
	{
		success = ramd_query_result_ok(result);
		PQclear(result);
		return success;
	}
...
char *
ramd_query_get_version(PGconn *conn)
{
	if (!conn)
		return NULL;
...
char *
ramd_query_get_current_wal_lsn(PGconn *conn)
{
	if (!conn)
		return NULL;
...
double
ramd_query_get_replication_lag(PGconn *conn)
{
	PGresult    *result;
	double       lag = -1.0;
	const char  *lag_str;
...
bool
ramd_query_has_active_transactions(PGconn *conn)
{
	PGresult *result;
	...
	if (result && PQntuples(result) > 0)
	{
		active_count = ramd_query_get_int_value(result, 0, 0);
...
bool
ramd_query_reload_config(PGconn *conn)
{
	if (!conn)
		return false;
