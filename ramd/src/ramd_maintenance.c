                                    ...
                                    size_t backup_id_size)
{
	...
	if (backup_dir == NULL) {
		backup_dir = "{{VAR_DIR}}lib/postgresql/backups";
	}
                                                     ...
                                                     int32_t primary_port)
{
	...
	if (use_recovery_conf)
	{
		...
		fprintf(
		    recovery_file,
		    "primary_conninfo = 'host=%s port=%d user=${POSTGRESQL_USER} password='\n",
                                             ...
                                             int32_t primary_port)
{
	...
	if (!network_range || strlen(network_range) == 0)
		network_range = RAMD_DEFAULT_NETWORK_RANGE; /* Default to 127.0.0.1 only
		                                               for security */
                                    ...
                                    int32_t replica_port, int32_t node_id)
{
	...
	fprintf(auto_conf,
	        "primary_conninfo = 'host=%s port=%d user=%s dbname=${POSTGRESQL_DATABASE}'\n",
