...
bool
ramd_maintenance_enter(const ramd_maintenance_config_t *config)
{
    ...
    {
        ramd_log_error("Invalid target node ID for maintenance: %d", config->target_node_id);
        return false;
    }
    ...
    {
        ramd_log_error("Pre-maintenance checks failed: %s", checks.check_details);
        return false;
    }
    ...
    {
        ramd_log_warning("Node %d is already in maintenance mode", config->target_node_id);
        pthread_mutex_unlock(&g_maintenance_mutex);
        return false;
    }
    ...
    strncpy(state->contact_info, config->contact_info, sizeof(state->contact_info) - 1);
    snprintf(state->initiated_by, sizeof(state->initiated_by), "ramd_pid_%d", getpid());
    state->auto_failover_disabled = false;
    state->connections_drained = false;
    
    pthread_mutex_unlock(&g_maintenance_mutex);
    
    ramd_log_info("Entering maintenance mode for node %d (type: %s, reason: %s)",
                  config->target_node_id,
                  ramd_maintenance_type_to_string(config->type),
                  config->reason);
    ...
    if (config->disable_auto_failover)
    {
        ...
        {
            ramd_log_warning("Failed to disable auto-failover for node %d", config->target_node_id);
        }
    }
    ...
    {
        char backup_id[64];
        if (ramd_maintenance_create_backup(config->target_node_id, backup_id, sizeof(backup_id)))
        {
            pthread_mutex_lock(&g_maintenance_mutex);
    ...
    {
        ramd_log_info("Draining connections for node %d", config->target_node_id);
        
        pthread_mutex_lock(&g_maintenance_mutex);
        state->status = RAMD_MAINTENANCE_STATUS_DRAINING;
        pthread_mutex_unlock(&g_maintenance_mutex);
        
        if (!ramd_maintenance_drain_connections(config->target_node_id, config->drain_timeout_ms))
        {
            ramd_log_warning("Failed to drain all connections for node %d", config->target_node_id);
        }
    }
    ...
    strncpy(state->status_message, "Maintenance mode active", sizeof(state->status_message) - 1);
    pthread_mutex_unlock(&g_maintenance_mutex);
    
    ramd_log_info("Node %d is now in active maintenance mode", config->target_node_id);
    return true;
}

bool
ramd_maintenance_exit(int32_t node_id)
{
    ramd_maintenance_state_t *state;
    ...
    strncpy(state->status_message, "Exiting maintenance mode", sizeof(state->status_message) - 1);
    ...
    if (state->auto_failover_disabled)
    {
        ...
        {
            ramd_log_warning("Failed to re-enable auto-failover for node %d", node_id);
        }
    }
    ...
    {
        ramd_log_warning("Failed to re-enable connections for node %d", node_id);
    }
...
bool
ramd_maintenance_get_status(int32_t node_id, ramd_maintenance_state_t *status)
{
    if (node_id <= 0 || node_id > RAMD_MAX_NODES || !status || !g_maintenance_initialized)
        return false;
        
    pthread_mutex_lock(&g_maintenance_mutex);
    memcpy(status, &g_maintenance_states[node_id - 1], sizeof(ramd_maintenance_state_t));
    pthread_mutex_unlock(&g_maintenance_mutex);
...
bool
ramd_maintenance_pre_check(const ramd_maintenance_config_t *config, ramd_maintenance_check_t *checks)
{
    if (!config || !checks)
        return false;
    ...
    if (!ramd_maintenance_get_connection_count(config->target_node_id, &checks->active_connections))
    {
...
bool
ramd_maintenance_is_safe_to_enter(const ramd_maintenance_config_t *config)
{
    ramd_maintenance_check_t checks;
...
bool
ramd_maintenance_drain_connections(int32_t node_id, int32_t timeout_ms)
{
    int32_t connection_count;
    int32_t elapsed_ms = 0;
    const int32_t check_interval_ms = 1000;
    
    ramd_log_info("Draining connections for node %d (timeout: %d ms)", node_id, timeout_ms);
    
    /* Prevent new connections */
    if (!ramd_maintenance_prevent_new_connections(node_id, true))
    {
        ramd_log_error("Failed to prevent new connections for node %d", node_id);
        return false;
    }
    ...
    while (elapsed_ms < timeout_ms)
    {
        ...
        {
            ramd_log_warning("Failed to get connection count for node %d", node_id);
            break;
        }
        
        if (connection_count <= 1) /* Only our own connection should remain */
        {
            ramd_log_info("Successfully drained connections for node %d", node_id);
        ...
        ramd_log_debug("Node %d still has %d active connections", node_id, connection_count);
        
        usleep((useconds_t)(check_interval_ms * 1000));
        elapsed_ms += check_interval_ms;
    }
    
    ramd_log_warning("Connection drain timeout for node %d (%d connections remaining)", 
                     node_id, connection_count);
    return false;
}

bool
ramd_maintenance_get_connection_count(int32_t node_id __attribute__((unused)), int32_t *count)
{
    PGconn *conn;
    PGresult *res;
...
bool
ramd_maintenance_prevent_new_connections(int32_t node_id, bool prevent)
{
    PGconn *conn;
    PGresult *res;
    char sql[256];
...
bool
ramd_maintenance_disable_auto_failover(int32_t node_id)
{
    pthread_mutex_lock(&g_maintenance_mutex);
    if (node_id <= RAMD_MAX_NODES)
    {
...
bool
ramd_maintenance_enable_auto_failover(int32_t node_id)
{
    pthread_mutex_lock(&g_maintenance_mutex);
    if (node_id <= RAMD_MAX_NODES)
    {
...
bool
ramd_maintenance_is_auto_failover_disabled(int32_t node_id)
{
    bool disabled = false;
...
bool
ramd_maintenance_create_backup(int32_t node_id, char *backup_id, size_t backup_id_size)
{
...
const char *
ramd_maintenance_type_to_string(ramd_maintenance_type_t type)
{
    switch (type)
    {
...
const char *
ramd_maintenance_status_to_string(ramd_maintenance_status_t status)
{
    switch (status)
    {
...
ramd_maintenance_string_to_type(const char *type_str)
{
    if (!type_str)
...
bool
ramd_maintenance_is_cluster_safe_for_maintenance(int32_t target_node_id)
{
    ramd_maintenance_config_t config;
    ramd_maintenance_check_t checks;
...
bool ramd_maintenance_list_scheduled(ramd_maintenance_state_t *states, int32_t *count)
{
    (void)states;
    if (count) *count = 0;
    return true;
...
bool
ramd_maintenance_bootstrap_new_node(const ramd_config_t *config, 
                                   ...
                                   int32_t primary_port)
{
    ...
    {
        ramd_log_error("Failed to create node data directory: %s", strerror(errno));
        return false;
    }
    ...
    snprintf(initdb_cmd, sizeof(initdb_cmd),
             "initdb -D %s --auth=trust --encoding=UTF8 --locale=C", node_data_dir);
    ...
    snprintf(postgresql_conf_path, sizeof(postgresql_conf_path), "%s/postgresql.conf", node_data_dir);
    ...
    snprintf(pg_hba_conf_path, sizeof(pg_hba_conf_path), "%s/pg_hba.conf", node_data_dir);
    ...
    fprintf(pg_hba_conf, "host    replication     postgres        %s/32        trust\n", primary_host);
    fprintf(pg_hba_conf, "host    all             postgres        %s/32        trust\n", primary_host);
    
    fclose(pg_hba_conf);
    
    /* Step 6: Take base backup from primary */
    if (!ramd_maintenance_take_basebackup_from_primary(node_data_dir, primary_host, primary_port))
    {
    ...
    if (!ramd_maintenance_configure_recovery_for_primary(node_data_dir, primary_host, primary_port))
    {
...
bool
ramd_maintenance_take_basebackup_from_primary(const char *data_dir, 
                                             const char *primary_host,
                                             int32_t primary_port)
{
    ...
    ramd_log_info("Taking base backup from primary %s:%d to %s", primary_host, primary_port, data_dir);
    
    /* Build pg_basebackup command */
    char backup_cmd[2048];
    snprintf(backup_cmd, sizeof(backup_cmd),
             "pg_basebackup -h %s -p %d -U postgres -D %s -Fp -Xs -P -v --no-password",
...
bool
ramd_maintenance_configure_recovery_for_primary(const char *data_dir,
                                               const char *primary_host,
                                               int32_t primary_port)
{
    if (!data_dir || !primary_host)
    ...
    ramd_log_info("Configuring recovery for primary %s:%d in %s", primary_host, primary_port, data_dir);
    
    /* Check PostgreSQL version to determine configuration method */
    bool use_recovery_conf = true; /* Default for older versions */
    
    /* Try to detect PostgreSQL version by checking if postgresql.auto.conf exists */
    ...
    if (use_recovery_conf)
    {
        ...
        snprintf(recovery_conf_path, sizeof(recovery_conf_path), "%s/recovery.conf", data_dir);
        ...
        fprintf(recovery_file, "# Recovery configuration generated by RAM bootstrap\n");
        fprintf(recovery_file, "standby_mode = 'on'\n");
        fprintf(recovery_file, "primary_conninfo = 'host=%s port=%d user=postgres password='\n", 
    ...
    else
    {
        ...
        fprintf(auto_conf_file, "primary_conninfo = 'host=%s port=%d user=postgres password='\n", 
                primary_host, primary_port);
    ...
    snprintf(trigger_file, sizeof(trigger_file), "%s/failover.trigger", data_dir);
    unlink(trigger_file);
...
bool
ramd_maintenance_bootstrap_cluster(const ramd_config_t *config,
                                  const char *cluster_name,
                                  const char *primary_host,
                                  int32_t primary_port,
                                  const char **standby_hosts,
                                  int32_t *standby_ports,
                                  int32_t standby_count)
{
    if (!config || !cluster_name || !primary_host || !standby_hosts || !standby_ports)
        return false;
        
    ramd_log_info("Bootstrapping complete cluster: %s with %d standby nodes", cluster_name, standby_count);
    
    /* Step 1: Bootstrap primary node */
    if (!ramd_maintenance_bootstrap_primary_node(config, cluster_name, primary_host, primary_port))
    {
    ...
    for (int i = 0; i < standby_count; i++)
    {
        ...
        if (!ramd_maintenance_bootstrap_new_node(config, node_name, standby_hosts[i], 
                                               standby_ports[i], primary_host, primary_port))
        {
    ...
    if (!ramd_maintenance_start_postgresql_node(config, primary_host, primary_port))
    {
    ...
    for (int i = 0; i < standby_count; i++)
    {
        if (!ramd_maintenance_start_postgresql_node(config, standby_hosts[i], standby_ports[i]))
        {
    ...
    if (!ramd_maintenance_verify_cluster_health(config, primary_host, primary_port, 
                                               standby_hosts, standby_ports, standby_count))
    {
...
bool
ramd_maintenance_bootstrap_primary_node(const ramd_config_t *config,
                                       const char *cluster_name,
                                       const char *primary_host,
                                       int32_t primary_port)
{
    ...
    ramd_log_info("Bootstrapping primary node: %s (%s:%d)", cluster_name, primary_host, primary_port);
    ...
    {
        ramd_log_error("Failed to create primary data directory: %s", strerror(errno));
        return false;
    }
    ...
    char initdb_cmd[1024];
    snprintf(initdb_cmd, sizeof(initdb_cmd),
             "initdb -D %s --auth=trust --encoding=UTF8 --locale=C", primary_data_dir);
    ...
    char postgresql_conf_path[512];
    snprintf(postgresql_conf_path, sizeof(postgresql_conf_path), "%s/postgresql.conf", primary_data_dir);
    ...
    /* Step 5: Configure pg_hba.conf for replication */
    char pg_hba_conf_path[512];
    snprintf(pg_hba_conf_path, sizeof(pg_hba_conf_path), "%s/pg_hba.conf", primary_data_dir);
    ...
    fprintf(pg_hba_conf, "host    replication     postgres        0.0.0.0/0        trust\n");
    fprintf(pg_hba_conf, "host    all             postgres        0.0.0.0/0        trust\n");
    
    fclose(pg_hba_conf);
    
    ramd_log_info("Primary node bootstrap completed successfully: %s", cluster_name);
    return true;
}

/* Function to start PostgreSQL node */
bool
ramd_maintenance_start_postgresql_node(const ramd_config_t *config,
                                      const char *host,
                                      int32_t port)
{
    if (!config || !host)
    ...
    snprintf(start_cmd, sizeof(start_cmd),
             "pg_ctl -D %s -o \"-p %d\" start", config->postgresql_data_dir, port);
...
bool
ramd_maintenance_verify_cluster_health(const ramd_config_t *config,
                                      const char *primary_host,
                                      int32_t primary_port,
                                      const char **standby_hosts,
                                      int32_t *standby_ports,
                                      int32_t standby_count)
{
    ...
    for (int i = 0; i < standby_count; i++)
    {
        if (!ramd_maintenance_verify_node_health(standby_hosts[i], standby_ports[i]))
        {
            ramd_log_error("Standby node %d health check failed", i + 1);
            return false;
        }
        
        if (!ramd_maintenance_verify_replication_status(standby_hosts[i], standby_ports[i]))
        {
...
bool
ramd_maintenance_verify_node_health(const char *host, int32_t port)
{
    if (!host)
        return false;
    ...
    if (!res || PQntuples(res) == 0)
    {
        ramd_log_error("Failed to check recovery status from node %s:%d", host, port);
        PQclear(res);
        ramd_postgresql_disconnect(&conn);
        return false;
    }
...
bool
ramd_maintenance_verify_replication_status(const char *host, int32_t port)
{
    if (!host)
        return false;
    ...
    PGresult *res = PQexec((PGconn*)conn.connection, "SELECT pg_is_in_recovery()");
    if (!res || PQntuples(res) == 0)
    {
        ramd_log_error("Failed to check recovery status from node %s:%d", host, port);
        PQclear(res);
    ...
    if (!res || PQntuples(res) == 0)
    {
        ramd_log_error("Failed to get replication status from node %s:%d", host, port);
        PQclear(res);
        ramd_postgresql_disconnect(&conn);
        return false;
    }
