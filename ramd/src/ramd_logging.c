                 ...
                 bool log_to_file, bool log_to_syslog, bool log_to_console)
{
    ...
    {
        strncpy(g_ramd_logging.log_file, log_file, sizeof(g_ramd_logging.log_file) - 1);
        
        g_ramd_logging.log_fp = fopen(log_file, "a");
        if (!g_ramd_logging.log_fp)
        {
            fprintf(stderr, "Critical error - Unable to open log file for writing: %s\n", log_file);
            return false;
        }
    }
        ...
        const char *function, const char *format, ...)
{
    ...
    snprintf(full_message, sizeof(full_message),
            "%s [%s] %s:%d %s(): %s",
            timestamp,
            ramd_logging_level_to_string(level),
            file, line, function,
            message);
