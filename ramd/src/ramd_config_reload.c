...
bool
ramd_config_reload_init(void)
{
    pthread_mutex_lock(&g_config_mutex);
    
    if (g_ramd_daemon && g_ramd_daemon->config_file)
    {
        strncpy(g_config_file_path, g_ramd_daemon->config_file, sizeof(g_config_file_path) - 1);
        memcpy(&g_current_config, &g_ramd_daemon->config, sizeof(ramd_config_t));
    }
...
void
ramd_config_reload_cleanup(void)
{
    pthread_mutex_lock(&g_config_mutex);
...
bool
ramd_config_reload_from_file(const char *config_file, ramd_config_reload_result_t *result)
{
    ...
    if (!ramd_config_validate_reload(&new_config, result->error_message, sizeof(result->error_message)))
    {
        result->status = RAMD_CONFIG_RELOAD_FAILED;
        pthread_mutex_unlock(&g_config_mutex);
        return false;
...
ramd_config_compare(const ramd_config_t *old_config, const ramd_config_t *new_config)
{
...
bool
ramd_config_validate_reload(const ramd_config_t *new_config, char *error_msg, size_t error_size)
{
    if (!new_config)
    {
        if (error_msg)
...
bool
ramd_config_reload_logging(const ramd_config_t *old_config, const ramd_config_t *new_config)
{
    (void)old_config;
    
    /* Reinitialize logging with new configuration */
    if (!ramd_logging_init(new_config->log_file, new_config->log_level,
                          strlen(new_config->log_file) > 0, new_config->log_to_syslog, 
...
bool
ramd_config_reload_monitoring(const ramd_config_t *old_config, const ramd_config_t *new_config)
{
    if (!old_config || !new_config)
        return false;
    ...
    if (old_config->monitor_interval_ms != new_config->monitor_interval_ms)
    {
        ramd_log_info("Monitor interval changed from %d ms to %d ms",
                      old_config->monitor_interval_ms, new_config->monitor_interval_ms);
    }
...
bool
ramd_config_reload_failover(const ramd_config_t *old_config, const ramd_config_t *new_config)
{
    if (!old_config || !new_config)
        return false;
    ...
    if (old_config->auto_failover_enabled != new_config->auto_failover_enabled)
    {
        ramd_log_info("Auto-failover %s", 
                      new_config->auto_failover_enabled ? "enabled" : "disabled");
    }
...
bool
ramd_config_reload_sync_replication(const ramd_config_t *old_config, const ramd_config_t *new_config)
{
    if (!old_config || !new_config)
        return false;
    ...
    if (old_config->synchronous_replication != new_config->synchronous_replication)
    {
        ramd_sync_mode_t new_mode = new_config->synchronous_replication ? 
                                   RAMD_SYNC_REMOTE_APPLY : RAMD_SYNC_OFF;
...
void
ramd_config_reload_signal_setup(void)
{
    struct sigaction sa;
...
void
ramd_config_reload_signal_sighup(int sig)
{
    (void)sig;
