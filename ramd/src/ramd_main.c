...
ramd_init(const char *config_file)
{
	...
	{
		ramd_log_error("Initialization failure: Unable to initialize daemon configuration subsystem");
		ramd_cleanup();
		return false;
	}
	...
	{
		ramd_log_error("Configuration error: Unable to load configuration file '%s' - please verify file exists and permissions", config_file);
		ramd_cleanup();
		return false;
	}
    ...
    {
        ramd_log_error("Cluster initialization failure: Unable to initialize cluster management subsystem");
        ramd_cleanup();
        return false;
    }
...
main(int argc, char *argv[])
{
	...
	if (log_file)
		strncpy(g_ramd_daemon->config.log_file, log_file, sizeof(g_ramd_daemon->config.log_file) - 1);
	if (pid_file)
		strncpy(g_ramd_daemon->config.pid_file, pid_file, sizeof(g_ramd_daemon->config.pid_file) - 1);
	if (log_level_str)
