...
ramd_http_server_init(ramd_http_server_t *server, const char *bind_address, int port)
{
    ...
    strncpy(server->bind_address, bind_address ? bind_address : "0.0.0.0", 
...
ramd_http_send_response(int client_fd, ramd_http_response_t *response)
{
    ...
    response_len = snprintf(response_buffer, sizeof(response_buffer),
        ...
        response->status == RAMD_HTTP_500_INTERNAL_ERROR ? "Internal Server Error" : "Unknown",
...
void
ramd_http_set_json_response(ramd_http_response_t *response, ramd_http_status_code_t status, const char *json)
{
    if (!response)
        return;
...
void
ramd_http_set_error_response(ramd_http_response_t *response, ramd_http_status_code_t status, const char *message)
{
...
void
ramd_http_handle_promote_node(ramd_http_request_t *request, ramd_http_response_t *response)
{
    char json_buffer[1024];
...
void
ramd_http_handle_demote_node(ramd_http_request_t *request, ramd_http_response_t *response)
{
    char json_buffer[1024];
...
void
ramd_http_handle_node_detail(ramd_http_request_t *request, ramd_http_response_t *response)
{
    char json_buffer[1024];
...
void
ramd_http_handle_maintenance_mode(ramd_http_request_t *request, ramd_http_response_t *response)
{
    char json_buffer[1024];
...
char *
ramd_http_get_query_param(const char *query_string, const char *param_name)
{
...
bool
ramd_http_authenticate(const char *authorization, const char *required_token)
{
