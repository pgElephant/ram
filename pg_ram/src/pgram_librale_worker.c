...
void
pgram_librale_worker_register(void)
{
    ...
    strlcpy(worker.bgw_function_name, "librale_worker_main", sizeof(worker.bgw_function_name));
...
void
librale_worker_main(Datum main_arg)
{
    (void) main_arg;
    ...
    /* Override default config inside pgram_librale_config using GUCs if provided */
    if (pg_ram_librale_config && pg_ram_librale_config->librale_config)
    {
        ...
        if (pgram_dstore_port > 0)
            (void) librale_config_set_dstore_port(cfg, (uint16)pgram_dstore_port);
...
static void
librale_worker_sigterm(SIGNAL_ARGS)
{
...
static void
librale_worker_sighup(SIGNAL_ARGS)
{
    (void) postgres_signal_arg;
    lib_got_sighup = true;
    SetLatch(&MyProc->procLatch);
