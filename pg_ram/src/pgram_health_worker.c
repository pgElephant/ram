...
void
pgram_health_status_snapshot(health_worker_status_t *out)
{
    if (!out)
        return;
    *out = g_health_status;
...
void
pgram_health_worker_register(void)
{
    ...
    strlcpy(worker.bgw_function_name, "health_worker_main", sizeof(worker.bgw_function_name));
...
void
health_worker_main(Datum main_arg)
{
    (void) main_arg;
    ...
    DefineCustomIntVariable("pg_ram.health_check_period_ms",
                            "Health worker sleep between checks (ms)",
                            NULL,
                            &pgram_health_period_ms,
    ...
    DefineCustomBoolVariable("pg_ram.health_verbose",
                             "Enable verbose health logging",
                             NULL,
                             &pgram_health_verbose,
                             false,
                             PGC_POSTMASTER,
                             0,
                             NULL, NULL, NULL);
    ...
    while (!got_sigterm)
    {
        ...
        {
            elog(LOG, "pg_ram: Health worker shutting down due to postmaster death");
            break;
        }
...
static void
health_worker_sigterm(SIGNAL_ARGS)
{
...
static void
health_worker_sighup(SIGNAL_ARGS)
{
    (void) postgres_signal_arg;
    got_sighup = true;
    SetLatch(&MyProc->procLatch);
