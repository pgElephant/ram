...
bool
quorum_selection_init(void)
{
    if (quorum_selection_config != NULL && quorum_selection_config->enabled)
        return true;
        
    quorum_selection_config = (quorum_selection_config_t *) 
        palloc0(sizeof(quorum_selection_config_t));
    
    if (quorum_selection_config == NULL)
    {
        elog(ERROR, "pg_ram: Failed to allocate quorum selection configuration");
        return false;
    }
    ...
    quorum_selection_config->dstore_port = (pgram_dstore_port > 0) ? pgram_dstore_port : 7401;
    ...
    if (pgram_node_ip && pgram_node_ip[0] != '\0')
        snprintf(quorum_selection_config->dstore_host, 
                sizeof(quorum_selection_config->dstore_host), "%s", pgram_node_ip);
    else
        snprintf(quorum_selection_config->dstore_host, 
                sizeof(quorum_selection_config->dstore_host), "localhost");
    
    elog(LOG, "pg_ram: Quorum selection initialized - node_id: %d, cluster: %s", 
         quorum_selection_config->node_id, quorum_selection_config->cluster_name);
    
    return true;
}

void
quorum_selection_cleanup(void)
{
    if (quorum_selection_config == NULL)
        return;
...
bool
quorum_selection_register_node(int32_t node_id, const char *hostname, int32_t port)
{
    if (quorum_selection_config == NULL || !quorum_selection_config->enabled)
        return false;
    ...
    {
        librale_status_t result = pgram_librale_add_node(node_id, hostname, hostname, 
                                                        7400, /* rale_port */
                                                        quorum_selection_config->dstore_port);
        if (result != LIBRALE_SUCCESS)
        {
            elog(WARNING, "pg_ram: Failed to register node %d via librale: %d", node_id, result);
            return false;
        }
    }
...
bool
quorum_selection_unregister_node(int32_t node_id)
{
    if (quorum_selection_config == NULL || !quorum_selection_config->enabled)
        return false;
    ...
    if (pg_ram_librale_config != NULL && pg_ram_librale_config->initialized)
    {
        ...
        if (result != LIBRALE_SUCCESS)
        {
            elog(WARNING, "pg_ram: Failed to unregister node %d via librale: %d", node_id, result);
            return false;
        }
    }
...
bool
quorum_selection_update_node_health(int32_t node_id, float health_score)
{
    if (quorum_selection_config == NULL || !quorum_selection_config->enabled)
        return false;
    ...
    elog(DEBUG1, "pg_ram: Updated health score for node %d: %.2f", node_id, health_score);
    return true;
}

bool
quorum_selection_check_quorum(quorum_decision_t *decision_out)
{
    if (quorum_selection_config == NULL || !quorum_selection_config->enabled)
        return false;
    ...
    if (pg_ram_librale_config != NULL && pg_ram_librale_config->initialized)
    {
        ...
        decision_out->healthy_nodes = decision_out->total_nodes;  /* Simplified */
        decision_out->votes_received = decision_out->healthy_nodes;
        
        /* Set decision reason */
        if (decision_out->has_quorum)
        {
            snprintf(decision_out->decision_reason, sizeof(decision_out->decision_reason),
...
bool
quorum_selection_is_leader(void)
{
    if (quorum_selection_config == NULL || !quorum_selection_config->enabled)
        return false;
...
int32_t
quorum_selection_get_leader_id(void)
{
    if (quorum_selection_config == NULL || !quorum_selection_config->enabled)
        return -1;
...
bool
quorum_selection_start_election(void)
{
    if (quorum_selection_config == NULL || !quorum_selection_config->enabled)
        return false;
...
bool
quorum_selection_become_leader(void)
{
    if (quorum_selection_config == NULL || !quorum_selection_config->enabled)
        return false;
...
bool
quorum_selection_step_down_leader(void)
{
    if (quorum_selection_config == NULL || !quorum_selection_config->enabled)
        return false;
...
quorum_selection_get_cluster_state(quorum_cluster_state_t *state_out)
{
    ...
    snprintf(state_out->cluster_id, sizeof(state_out->cluster_id), 
            "%s", quorum_selection_config->cluster_name);
    ...
    if (pg_ram_librale_config != NULL && pg_ram_librale_config->initialized)
    {
        ...
        state_out->standby_count = state_out->node_count - state_out->primary_count;
    }
...
bool
quorum_selection_should_trigger_failover(void)
{
...
char*
quorum_selection_get_status_summary(void)
{
    quorum_decision_t decision;
    char *summary;
...
bool quorum_selection_vote_for_leader(int32_t candidate_node_id)
{
    ...
    if (pg_ram_librale_config != NULL && pg_ram_librale_config->initialized)
    {
        librale_status_t result = pgram_librale_vote_for_leader(candidate_node_id);
        if (result != LIBRALE_SUCCESS)
        {
...
bool quorum_selection_propose_value(const char *key, const char *value)
{
    ...
    if (pg_ram_librale_config != NULL && pg_ram_librale_config->initialized)
    {
        ...
        if (result != LIBRALE_SUCCESS)
        {
            elog(WARNING, "pg_ram: Failed to propose value for key '%s': %d", key, result);
            return false;
        }
...
bool quorum_selection_get_consensus_value(const char *key, char *value_out, size_t value_size)
{
    if (quorum_selection_config == NULL || !quorum_selection_config->enabled)
        return false;
    ...
    if (pg_ram_librale_config != NULL && pg_ram_librale_config->initialized)
    {
        ...
        if (len >= value_size)
        {
            elog(WARNING, "pg_ram: Consensus value too large for buffer (need %zu, have %zu)", 
                 len + 1, value_size);
...
bool quorum_selection_delete_consensus_value(const char *key)
{
    ...
    if (pg_ram_librale_config != NULL && pg_ram_librale_config->initialized)
    {
        ...
        if (result != LIBRALE_SUCCESS)
        {
            elog(WARNING, "pg_ram: Failed to delete consensus value for key '%s': %d", key, result);
            return false;
        }
...
bool quorum_selection_trigger_failover(int32_t target_node_id)
{
    ...
    if (pg_ram_librale_config != NULL && pg_ram_librale_config->initialized)
    {
        librale_status_t result = pgram_librale_trigger_failover(target_node_id);
        if (result != LIBRALE_SUCCESS)
        {
...
bool quorum_selection_detect_split_brain(void)
{
    ...
    if (pg_ram_librale_config != NULL && pg_ram_librale_config->initialized)
    {
        ...
        if (leader_count > 1)
        {
            elog(WARNING, "pg_ram: Split-brain detected - multiple leaders (%d)", leader_count);
            return true;
        }
...
bool quorum_selection_resolve_split_brain(void)
{
    ...
    if (pg_ram_librale_config != NULL && pg_ram_librale_config->initialized)
    {
        ...
        if (result != LIBRALE_SUCCESS)
        {
            elog(ERROR, "pg_ram: Failed to resolve split-brain via leader election: %d", result);
            return false;
        }
