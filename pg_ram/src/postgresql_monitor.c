...
bool postgresql_monitor_init(void)
{
	if (postgresql_monitor_config != NULL && postgresql_monitor_config->enabled)
		return true;

	postgresql_monitor_config = (postgresql_monitor_config_t*) palloc0(
	    sizeof(postgresql_monitor_config_t));

	if (postgresql_monitor_config == NULL)
	{
		elog(ERROR,
		     "pg_ram: Failed to allocate PostgreSQL monitor configuration");
		return false;
	}
...
void postgresql_monitor_cleanup(void)
{
	if (postgresql_monitor_config == NULL)
		return;
...
bool postgresql_monitor_health_check(postgresql_health_t* health_out)
{
	if (postgresql_monitor_config == NULL ||
	    !postgresql_monitor_config->enabled)
	{
	...
	if (health_out->max_connections > 0)
	{
		health_out->connection_usage_percentage =
		    (float) health_out->active_connections /
		    health_out->max_connections * 100.0;
	}
	...
	else
	{
		    ...
		    !XLogRecPtrIsInvalid(health_out->replayed_lsn))
		{
			health_out->wal_lag_seconds =
			    (int32_t) ((health_out->received_lsn -
			                health_out->replayed_lsn) /
	...
	if (health_out->overall_health_score >= 0.9)
		...
		snprintf(
		    health_out->status_message, sizeof(health_out->status_message),
		    "PostgreSQL has critical issues requiring immediate attention");

	return true;
}

bool postgresql_monitor_is_healthy(void)
{
...
float postgresql_monitor_get_health_score(void)
{
	float score = 0.0;
	int factors = 0;
...
bool postgresql_monitor_can_accept_writes(void)
{
	postgresql_health_t health;
	...
	return (health.is_running && health.is_accepting_connections &&
	        health.is_primary && health.connection_usage_percentage < 95.0);
}

bool postgresql_monitor_is_replication_healthy(void)
{
	if (RecoveryInProgress())
	{
		...
		if (XLogRecPtrIsInvalid(received_lsn) ||
		    XLogRecPtrIsInvalid(replayed_lsn))
			return false;
...
bool postgresql_monitor_check_wal_health(void)
{
...
bool postgresql_monitor_check_disk_space(void)
{
...
char* postgresql_monitor_get_status_summary(void)
{
	postgresql_health_t health;
	char* summary;
	...
	snprintf(
	    ...
	    health.is_running ? "Running" : "Stopped", health.overall_health_score,
	    health.is_primary ? "Primary" : "Standby", health.active_connections,
	    health.max_connections, health.connection_usage_percentage,
...
bool postgresql_monitor_get_replication_status(char* status_out,
                                               size_t status_size)
{
...
bool postgresql_monitor_get_connection_info(char* info_out, size_t info_size)
{
