...
pgram_librale_init(void)
{
	...
	pg_ram_librale_config = (pg_ram_librale_config_t *) palloc0(sizeof(pg_ram_librale_config_t));
	...
	pg_ram_librale_config->node_id = (pgram_node_id > 0) ? pgram_node_id : DEFAULT_NODE_ID;

	if (pgram_node_name && pgram_node_name[0] != '\0')
		snprintf(pg_ram_librale_config->node_name, sizeof(pg_ram_librale_config->node_name),
				 "%s", pgram_node_name);
	else
		snprintf(pg_ram_librale_config->node_name, sizeof(pg_ram_librale_config->node_name),
				 "pg_ram_node_%d", pg_ram_librale_config->node_id);

	if (pgram_node_ip && pgram_node_ip[0] != '\0')
		snprintf(pg_ram_librale_config->node_ip, sizeof(pg_ram_librale_config->node_ip),
				 "%s", pgram_node_ip);
	else
		snprintf(pg_ram_librale_config->node_ip, sizeof(pg_ram_librale_config->node_ip),
				 "127.0.0.1");

	pg_ram_librale_config->rale_port = (uint16)
		((pgram_rale_port >= 1 && pgram_rale_port <= 65535) ? pgram_rale_port : DEFAULT_RALE_PORT);
	pg_ram_librale_config->dstore_port = (uint16)
		((pgram_dstore_port >= 1 && pgram_dstore_port <= 65535) ? pgram_dstore_port : DEFAULT_DSTORE_PORT);	if (pgram_db_path && pgram_db_path[0] != '\0')
	{
		snprintf(pg_ram_librale_config->db_path, sizeof(pg_ram_librale_config->db_path),
				 "%s", pgram_db_path);
		snprintf(pg_ram_librale_config->log_directory, sizeof(pg_ram_librale_config->log_directory),
				 "%s", pgram_db_path);
	}
	else
	{
    ...
    result = librale_config_set_node_id(librale_config, pg_ram_librale_config->node_id);
    ...
    result = librale_config_set_node_name(librale_config, pg_ram_librale_config->node_name);
    if (result != LIBRALE_OK)
    {
    ...
    result = librale_config_set_node_ip(librale_config, pg_ram_librale_config->node_ip);
    if (result != LIBRALE_OK)
    {
    ...
    result = librale_config_set_rale_port(librale_config, pg_ram_librale_config->rale_port);
    if (result != LIBRALE_OK)
    {
    ...
    result = librale_config_set_dstore_port(librale_config, pg_ram_librale_config->dstore_port);
    if (result != LIBRALE_OK)
    {
    ...
    result = librale_config_set_db_path(librale_config, pg_ram_librale_config->db_path);
    if (result != LIBRALE_OK)
    {
    ...
    result = librale_config_set_log_directory(librale_config, pg_ram_librale_config->log_directory);
    if (result != LIBRALE_OK)
    {
    ...
    result = librale_dstore_init(pg_ram_librale_config->dstore_port, librale_config);
    if (result != LIBRALE_OK)
    {
        elog(ERROR, "pg_ram: Failed to initialize librale distributed store: %d", result);
        librale_rale_finit();
        librale_config_destroy(librale_config);
        pfree(pg_ram_librale_config);
        pg_ram_librale_config = NULL;
    ...
    elog(LOG, "pg_ram: librale initialized successfully for node %d (%s) at %s:%d/%d",
...
void
pgram_librale_cleanup(void)
{
    if (pg_ram_librale_config == NULL || !pg_ram_librale_config->initialized)
        return;
    
    librale_dstore_finit(NULL, 0);
    librale_rale_finit();
...
int32
pgram_librale_get_leader_id(void)
{
	if (pg_ram_librale_config == NULL || !pg_ram_librale_config->initialized)
		return -1;
...
uint32
pgram_librale_get_node_count(void)
{
	if (pg_ram_librale_config == NULL || !pg_ram_librale_config->initialized)
		return 0;
...
librale_status_t
pgram_librale_add_node(int32 node_id, const char *name, const char *ip,
					   uint16 rale_port, uint16 dstore_port)
{
	if (pg_ram_librale_config == NULL || !pg_ram_librale_config->initialized)
		return LIBRALE_ERR_INVALID;
...
librale_status_t
pgram_librale_remove_node(int32 node_id)
{
	if (pg_ram_librale_config == NULL || !pg_ram_librale_config->initialized)
		return LIBRALE_ERR_INVALID;
...
librale_status_t
pgram_librale_process_consensus(void)
{
	if (pg_ram_librale_config == NULL || !pg_ram_librale_config->initialized)
		return LIBRALE_ERR_INVALID;
...
int32
pgram_librale_get_current_role(void)
{
	if (pg_ram_librale_config == NULL || !pg_ram_librale_config->initialized)
		return -1;
...
int32
pgram_librale_get_current_term(void)
{
	return 0;
}

bool
pgram_librale_is_node_healthy(int32 node_id)
{
	if (pg_ram_librale_config == NULL || !pg_ram_librale_config->initialized)
		return false;
...
librale_status_t
pgram_librale_get_cluster_status(char *status, size_t status_size)
{
	if (pg_ram_librale_config == NULL || !pg_ram_librale_config->initialized)
		return LIBRALE_ERR_INVALID;
	...
	snprintf(status, status_size,
			 ...
			 pgram_librale_get_current_role(),
			 pgram_librale_get_leader_id(),
			 pgram_librale_get_node_count(),
