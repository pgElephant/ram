...
bool postgresql_healthcheck_init(void)
{
	if (postgresql_healthcheck_config != NULL &&
	    postgresql_healthcheck_config->enabled)
		return true;

	postgresql_healthcheck_config = (postgresql_healthcheck_config_t*) palloc0(
	    sizeof(postgresql_healthcheck_config_t));

	if (postgresql_healthcheck_config == NULL)
	{
		elog(ERROR,
		     "pg_ram: Failed to allocate PostgreSQL healthcheck configuration");
		return false;
	}
...
void postgresql_healthcheck_cleanup(void)
{
	if (postgresql_healthcheck_config == NULL)
		return;
...
bool postgresql_healthcheck_run_all(postgresql_healthcheck_status_t* status_out)
{
	...
	if (postgresql_healthcheck_connection_limits(
	        &status_out->checks[check_idx]))
	{
		status_out->passed_checks++;
	}
	...
	status_out->overall_score =
	    postgresql_healthcheck_calculate_score(status_out);

	return true;
}

bool postgresql_healthcheck_database_connectivity(
    postgresql_healthcheck_result_t* result_out)
{
	...
	else
	{
		...
		strcpy(result_out->error_message,
...
bool postgresql_healthcheck_replication_lag(
    postgresql_healthcheck_result_t* result_out)
{
	if (result_out == NULL)
		return false;
...
bool postgresql_healthcheck_connection_limits(
    postgresql_healthcheck_result_t* result_out)
{
	if (result_out == NULL)
		return false;
	...
	else
	{
		...
		strcpy(result_out->error_message,
		       "Connection usage is approaching limits");
	}
    ...
    postgresql_healthcheck_result_t* result_out)
{
	...
	appendStringInfo(
	    &query, "SELECT pg_size_pretty(pg_database_size(current_database())) "
	            "as db_size, "
	            "pg_size_pretty(pg_total_relation_size('pg_stat_activity')) as "
	            "sys_size");
	...
	if (SPI_connect() != SPI_OK_CONNECT)
	{
		result_out->passed = false;
		strcpy(result_out->error_message,
		       "Failed to connect to database for disk space check");
		return false;
	}
    ...
    postgresql_healthcheck_result_t* result_out)
{
	...
	if (SPI_connect() != SPI_OK_CONNECT)
	{
		result_out->passed = false;
		strcpy(result_out->error_message,
		       "Failed to connect to database for memory check");
		return false;
	}
    ...
    postgresql_healthcheck_result_t* result_out)
{
	...
	if (SPI_connect() != SPI_OK_CONNECT)
	{
		result_out->passed = false;
		strcpy(result_out->error_message,
		       "Failed to connect to database for WAL archiving check");
		return false;
	}
	...
	if (ret == SPI_OK_SELECT && SPI_processed > 0)
	{
		...
		if (!result_out->passed)
			strcpy(result_out->error_message,
			       "WAL archiving failures detected");
	}
	else
	{
		result_out->passed = false;
		strcpy(result_out->error_message,
		       "Failed to retrieve WAL archiving status");
	}
    ...
    postgresql_healthcheck_result_t* result_out)
{
	...
	if (SPI_connect() != SPI_OK_CONNECT)
	{
		result_out->passed = false;
		strcpy(result_out->error_message,
		       "Failed to connect to database for vacuum status check");
		return false;
	}
	...
	if (ret == SPI_OK_SELECT && SPI_processed > 0)
	{
		...
		if (!result_out->passed)
			strcpy(result_out->error_message,
			       "Too many tables without recent vacuum");
	}
	else
	{
		result_out->passed = false;
    ...
    postgresql_healthcheck_result_t* result_out)
{
	...
	if (SPI_connect() != SPI_OK_CONNECT)
	{
		result_out->passed = false;
		strcpy(result_out->error_message,
		       "Failed to connect to database for lock contention check");
		return false;
	}
	...
	else
	{
		result_out->passed = false;
		strcpy(result_out->error_message,
		       "Failed to retrieve lock contention information");
	}
    ...
    postgresql_healthcheck_result_t* result_out)
{
	...
	if (SPI_connect() != SPI_OK_CONNECT)
	{
		result_out->passed = false;
		strcpy(result_out->error_message,
		       "Failed to connect to database for checkpoint check");
		return false;
	}
	...
	else
	{
		result_out->passed = false;
		strcpy(result_out->error_message,
		       "Failed to retrieve checkpoint performance data");
	}
    ...
    postgresql_healthcheck_result_t* result_out)
{
	...
	if (SPI_connect() != SPI_OK_CONNECT)
	{
		result_out->passed = false;
		strcpy(result_out->error_message,
		       "Failed to connect to database for backup status check");
		return false;
	}
...
bool postgresql_healthcheck_run_single(
    const char* check_name, postgresql_healthcheck_result_t* result_out)
{
...
bool postgresql_healthcheck_is_critical_failure(
...
float postgresql_healthcheck_calculate_score(
    const postgresql_healthcheck_status_t* status)
{
