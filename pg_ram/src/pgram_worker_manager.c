...
void
pgram_worker_manager_init(void)
{
    ...
    elog(LOG, "pg_ram: Worker manager initialized with %d workers", worker_count);
}

static void
worker_manager_init(void)
{
    int i;                              /* Loop counter */
...
static void
register_worker(worker_definition_t *worker_def)
{
    ...
    else if (strcmp(worker_def->function, "health_worker_main") == 0)
    {
        ...
        elog(WARNING, "pg_ram: Unknown worker function: %s", worker_def->function);
}

static void __attribute__((used))
unregister_worker(worker_definition_t *worker_def)
{
    elog(DEBUG1, "pg_ram: Worker unregistration requested for: %s", worker_def->name);
}

void
pgram_worker_manager_cleanup(void)
{
    if (!workers_registered)
        return;
...
static void
worker_manager_cleanup(void)
{
    elog(DEBUG1, "pg_ram: Worker manager cleanup completed");
}

void
pgram_worker_manager_get_status(StringInfo buf)
{
    int i;                              /* Loop counter */
